...
class TempoCoordinationError(Exception):
    pass
...
def run_operation(operation: str | None, payload: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise TempoCoordinationError("Tempo coordination input must be a JSON object")
    ...
    raise TempoCoordinationError(f"Unsupported Tempo coordination operation: {op}")
...
def parse_payload(payload: Any) -> Any:
    ...
    if isinstance(payload, str):
        text = payload.strip()
        if not text:
            raise TempoCoordinationError("Tempo coordination input was empty")
...
def unwrap_payload(value: Any) -> Any:
    ...
    while queue:
        ...
        if isinstance(current, dict):
            if has_tempo_input_shape(current):
                return current
            fallback = current
...
def has_tempo_input_shape(value: dict[str, Any]) -> bool:
    return any(
        key in value
        for key in (
            "repoId",
        ...
        for right in parsed[index + 1 :]:
            ...
            for file_path in shared_files:
                ...
                if left["huge"] or right["huge"] or file_path in left["binary"] or file_path in right["binary"]:
                    ...
                    continue
                if final_file_hashes_match(left, right, file_path):
                    evidence.append(
                        merge_risk_evidence(
                            "Aligned final file",
                            f"Both worktrees now have identical final text for {file_path}.",
                            [file_path],
...
def parse_diff_input(value: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "fileHashes": normalize_file_hashes(value.get("fileHashes")),
        "files": sorted(files),
        "deleted": deleted,
        "binary": binary,
        "hunks": hunks,
...
def normalize_file_hashes(value: Any) -> dict[str, str]:
    if not isinstance(value, dict):
        return {}
    return {
        str(path): str(hash_value)
        for path, hash_value in value.items()
        if isinstance(path, str) and isinstance(hash_value, str) and hash_value
    }


def final_file_hashes_match(left: dict[str, Any], right: dict[str, Any], file_path: str) -> bool:
    left_hash = (left.get("fileHashes") or {}).get(file_path)
    right_hash = (right.get("fileHashes") or {}).get(file_path)
...
def is_generated_path(file_path: str) -> bool:
    return bool(
        re.search(
            r"(^|/)(node_modules|dist|build|coverage|\.next|\.turbo|\.tempo)/|(\.tsbuildinfo|package-lock\.json|pnpm-lock\.yaml|yarn\.lock)$",
            file_path,
        )
    )
...
def merge_risk_diff_hash(
    ...
    parts = [
        ...
        *sorted(
            f"{diff.get('worktreeId')}:{path}:{hash_value}"
            for diff in diffs
            for path, hash_value in normalize_file_hashes(diff.get("fileHashes")).items()
        ),
    ]
...
def build_merge_contract(
    ...
    return {
        ...
        **({"sourceSnapshotSetId": str(publication.get("snapshotSetId"))} if publication.get("snapshotSetId") else {}),
...
def build_work_order(
    ...
    required_feature_terms = feature_terms_for_agent(agent)
    required_snapshot_publication_id = (
        str(merge_contract.get("sourcePublicationId"))
        if role == "adapter" and merge_contract and merge_contract.get("sourcePublicationId")
        else None
    )
    blocked_files = [] if required_snapshot_publication_id else ([] if role == "contract_owner" else shared_files)
    return {
        ...
        "summary": (
            ...
            if role == "contract_owner"
                ...
                f"{owner_name} owns {episode['surface']}. Match the published owner snapshot for shared files, preserve this feature's required terms, then checkpoint."
                if required_snapshot_publication_id
                else f"{owner_name} owns {episode['surface']}. Pause shared-file edits until the owner publishes the canonical snapshot, then checkpoint."
            )
        ),
        "requiredContract": contract_summary,
        **({"requiredSnapshotPublicationId": required_snapshot_publication_id} if required_snapshot_publication_id else {}),
        "requiredFeatureTerms": required_feature_terms,
        ...
        "nextCheckpoint": (
            ...
            if role == "contract_owner"
                ...
                "Checkpoint after shared files match the owner snapshot and feature terms are still present."
                if required_snapshot_publication_id
                else "Wait for the owner snapshot before editing shared files."
            )
        ),
        "createdAt": created_at,
        "updatedAt": created_at,
...
def build_openai_coordination_plan(
    ...
    fixture = os.environ.get("TEMPO_OPENAI_PLANNER_FIXTURE")
    if not api_key:
        if mode == "required":
            raise TempoCoordinationError(
    ...
    try:
        ...
        if mode == "required":
            if isinstance(error, TempoCoordinationError):
                raise
            raise TempoCoordinationError(f"OpenAI coordination planner failed: {error}") from error
        return None
...
def request_openai_coordination_plan(
    ...
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise TempoCoordinationError("OPENAI_API_KEY is not configured")
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are Tempo's coordination planner for parallel coding agents. "
                            ...
                            "workOrders": [
                                {
                                    ...
                                    "requiredSnapshotPublicationId": "owner publication id when an adapter must match exact shared-file text",
                                    "requiredFeatureTerms": ["agent-specific feature terms that must remain present"],
    ...
    try:
        ...
        raise TempoCoordinationError(f"OpenAI planner HTTP {error.code}: {detail}") from error
    content = (((response_body.get("choices") or [{}])[0].get("message") or {}).get("content"))
    if not content:
        raise TempoCoordinationError("OpenAI planner response was empty")
    parsed = json.loads(content)
    if not isinstance(parsed, dict):
        raise TempoCoordinationError("OpenAI planner response was not an object")
    return parsed
...
def normalize_openai_coordination_plan(
    ...
    if not isinstance(raw_plan, dict):
        raise TempoCoordinationError("OpenAI coordination plan must be an object")
...
def apply_coordination_plan_to_work_orders(
    ...
    for raw_order in raw_orders:
        ...
        base_by_agent[agent_id] = {
            ...
            **({"requiredSnapshotPublicationId": str(base.get("requiredSnapshotPublicationId"))} if base.get("requiredSnapshotPublicationId") else {}),
            "requiredFeatureTerms": bounded_text_list(raw_order.get("requiredFeatureTerms") or base.get("requiredFeatureTerms"), 30),
...
ENFORCED_FEATURE_TERMS = ["label", "project", "subtitle", "reminderAt", "archived", "batchId"]


def feature_terms_for_agent(agent: dict[str, Any]) -> list[str]:
    haystack = " ".join(
        [
            str(agent.get("displayName") or ""),
            str(agent.get("currentPlan") or ""),
        ]
    )
    haystack_lower = haystack.lower()
    terms = []
    for term in ENFORCED_FEATURE_TERMS:
        if term.lower() in haystack_lower:
            terms.append(term)
    if "reminder" in haystack_lower:
        for term in ["subtitle", "reminderAt"]:
            if term not in terms:
                terms.append(term)
    if "bulk" in haystack_lower or "archive" in haystack_lower or "batch" in haystack_lower:
        for term in ["archived", "batchId"]:
            if term not in terms:
                terms.append(term)
    if "label" in haystack_lower or "project" in haystack_lower:
        for term in ["label", "project"]:
            if term not in terms:
                terms.append(term)
...
def require_text(value: dict[str, Any], key: str) -> str:
    ...
    if not isinstance(text, str) or not text:
        ...
        raise TempoCoordinationError(
            f"Tempo coordination input missing {key}; keys: {keys}"
        )
    return text
